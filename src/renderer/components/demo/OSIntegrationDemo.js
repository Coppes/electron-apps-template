...
export default function OSIntegrationDemo() {
    ...
    return (_jsxs("div", { className: "w-full", children: [_jsx("div", { className: "border-b border-gray-200 mb-6", children: _jsx("div", { className: "flex flex-wrap gap-2", children: tabs.map((tab) => {
                        const Icon = tab.icon;
                        return (_jsxs("button", { onClick: () => setActiveTab(tab.id), className: `flex items-center gap-2 px-4 py-3 border-b-2 transition-all ${activeTab === tab.id
                                ? 'border-blue-600 text-blue-600 font-medium'
                                : 'border-transparent text-gray-600 hover:text-gray-900 hover:border-gray-300'}`, children: [_jsx(Icon, { className: "w-4 h-4" }), tab.label] }, tab.id));
                    }) }) }), _jsx("div", { className: "animate-fadeIn", children: ActiveComponent && _jsx(ActiveComponent, {}) })] }));
}
