...
const TabBar = ({ group = 'primary' }) => {
    const { tabs: primaryTabs, activeTabId: primaryActiveId, secondaryTabs, secondaryActiveTabId, setActiveTab, closeTab, closeAllTabs, closeOtherTabs, addTab, reorderTab, moveTabToGroup, draggingTab, setDraggingTab } = useTabContext();
    ...
    const statusBarItem = React.useMemo(() => ({
        id: 'tab-count',
        content: _jsx("span", { className: "text-xs text-muted-foreground", children: t('status.tabs', { count: tabs.length }) }),
        position: 'right',
        priority: 10
    }), [tabs.length, t]);
    // Update Status Bar
    ...
    const getIcon = (id) => {
        if (id === 'home')
            return _jsx(House, { className: "w-4 h-4" });
        if (id === 'settings')
            return _jsx(Gear, { className: "w-4 h-4" });
        if (id === 'about')
            return _jsx(Info, { className: "w-4 h-4" });
        return _jsx(Cube, { className: "w-4 h-4" });
    };
    const [dragOverIndex, setDragOverIndex] = React.useState(null);
    return (_jsx("div", { className: cn("w-full bg-muted/40 border-b border-border transition-colors", draggingTab?.group === group && "bg-muted/60" // Feedback for active group
        ), onAuxClick: (e) => {
            ...
            if (tabId) {
                ...
                if (isInGroup) {
                    ...
                    moveTabToGroup(tabId, group);
        ...
        }, onDragLeave: () => setDragOverIndex(null), children: _jsxs("div", { className: "flex items-center w-full overflow-x-auto no-scrollbar", children: [tabs.map((tab, index) => (_jsxs(ContextMenu.ContextMenu, { children: [_jsx(ContextMenu.ContextMenuTrigger, { children: _jsxs("div", { draggable: true, onDragStart: (e) => {
                                ...
                                }, className: cn("group flex items-center gap-2 px-3 py-2 text-xs border-r border-border/50 cursor-pointer select-none transition-colors min-w-[120px] max-w-[200px]", activeTabId === tab.id
                                    ? "bg-background text-foreground border-b-2 border-b-primary font-medium"
                                    : "bg-muted/40 text-muted-foreground hover:bg-muted/60"), onClick: () => setActiveTab(tab.id), onAuxClick: (e) => {
                                    if (e.button === 1) {
                                        e.preventDefault();
                                        closeTab(tab.id);
                                    }
                                }, children: [getIcon(tab.id), _jsx("span", { className: "truncate flex-1", children: tab.type === 'home' ? t('nav.home') :
                                            tab.type === 'settings' ? t('nav.items.settings') :
                                                tab.type === 'demo' ? t('nav.items.legacy_demo') :
                                                    tab.type === 'test' ? t('nav.items.test_playground') :
                                                        tab.title }), _jsx("button", { className: cn("opacity-0 group-hover:opacity-100 hover:bg-destructive/10 hover:text-destructive rounded p-0.5 transition-all", activeTabId === tab.id && "opacity-100"), onClick: (e) => {
                                            e.stopPropagation();
                                            closeTab(tab.id);
                                        }, children: _jsx(X, { className: "w-3 h-3" }) })] }) }), _jsxs(ContextMenu.ContextMenuContent, { className: "min-w-[160px] bg-popover rounded-md border border-border p-1 shadow-md z-[50]", children: [_jsx(ContextMenu.ContextMenuItem, { onClick: () => addTab({ ...tab, id: `tab-${Date.now()}` }), children: "Duplicate Tab" }), _jsx(ContextMenu.ContextMenuSeparator, {}), _jsx(ContextMenu.ContextMenuItem, { onClick: () => closeTab(tab.id), children: "Close Tab" }), _jsx(ContextMenu.ContextMenuItem, { onClick: () => closeOtherTabs(tab.id), children: "Close Others" }), _jsx(ContextMenu.ContextMenuItem, { onClick: () => {
                                        const targetGroup = group === 'primary' ? 'secondary' : 'primary';
                                        moveTabToGroup(tab.id, targetGroup);
                                    }, children: "Split Right" }), _jsx(ContextMenu.ContextMenuSeparator, {}), _jsx(ContextMenu.ContextMenuItem, { className: "text-destructive focus:text-destructive", onClick: closeAllTabs, children: "Close All" })] })] }, tab.id))), _jsx("div", { className: cn("transition-all duration-200 ease-in-out w-0 overflow-hidden", (dragOverIndex === tabs.length &&
                        !(draggingTab?.group === group && draggingTab?.index === tabs.length - 1)) && "w-[120px] opacity-50 bg-primary/10 border-2 border-dashed border-primary/30 rounded mx-1"), style: { height: '32px' }, onDragOver: (e) => {
                        e.preventDefault();
                        e.stopPropagation();
                    ...
                    } }), _jsx("button", { className: "flex items-center justify-center px-3 py-2 text-muted-foreground hover:bg-muted/60 hover:text-foreground transition-colors border-r border-border/50", onClick: newTabAction, title: "New Tab (Ctrl+T)", onDragOver: (e) => {
                        e.preventDefault();
                        setDragOverIndex(tabs.length);
