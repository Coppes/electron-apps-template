...
export const ShortcutProvider = ({ children }) => {
    ...
    useEffect(() => {
        const handleKeyDown = (event) => {
            ...
            for (const shortcut of shortcuts) {
                ...
                if (hasShift && !requiredModifiers.shift)
                    continue;
                if (hasAlt && !requiredModifiers.alt)
                    continue;
                ...
                const isModifierAllowed = (mod) => {
                    if (mod === 'shift')
                        return allowedModifiers.has('shift');
                    if (mod === 'alt')
                        return allowedModifiers.has('alt');
                    if (mod === 'ctrl')
                        return allowedModifiers.has('ctrl') || allowedModifiers.has('control') || allowedModifiers.has('mod');
                    if (mod === 'meta')
                        return allowedModifiers.has('meta') || allowedModifiers.has('cmd') || allowedModifiers.has('mod');
                    return false;
                };
                const unexpectedModifier = eventModifiers.some(m => !isModifierAllowed(m));
                if (unexpectedModifier)
                    continue;
                if (unexpectedModifier)
                    continue;
    ...
    return (_jsx(ShortcutContext.Provider, { value: { shortcuts, registerShortcut, unregisterShortcut, updateShortcut, resetToDefaults, importOverrides, userOverrides }, children: children }));
};
