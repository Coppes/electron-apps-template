...
const shortcutsAPI = {
     ...
     * @returns {Promise<Object>} Result
     */
    register: (accelerator, description) => ipcRenderer.invoke(IPC_CHANNELS.SHORTCUT_REGISTER, { accelerator, description }),
    /**
     * Unregister a global shortcut
     * @param {string} accelerator - Keyboard accelerator
     * @returns {Promise<Object>} Result
     */
    unregister: (accelerator) => ipcRenderer.invoke(IPC_CHANNELS.SHORTCUT_UNREGISTER, { accelerator }),
    /**
     * Unregister all global shortcuts
     * @returns {Promise<Object>} Result
     */
    unregisterAll: () => ipcRenderer.invoke(IPC_CHANNELS.SHORTCUT_UNREGISTER_ALL, {}),
    /**
     * Check if shortcut is registered
     * @param {string} accelerator - Keyboard accelerator
     * @returns {Promise<boolean>} True if registered
     */
    isRegistered: (accelerator) => ipcRenderer.invoke(IPC_CHANNELS.SHORTCUT_IS_REGISTERED, { accelerator }).then(r => r.registered),
    /**
     * List all active shortcuts
     * @returns {Promise<import('./common/types.ts').ShortcutInfo[]>} Active shortcuts
     */
    listActive: () => ipcRenderer.invoke(IPC_CHANNELS.SHORTCUT_LIST_ACTIVE, {}).then(r => r.shortcuts),
    /**
...
const progressAPI = {
     ...
     * @returns {Promise<Object>} Result
     */
    set: (value, options = {}) => ipcRenderer.invoke(IPC_CHANNELS.PROGRESS_SET, { value, ...options }),
    /**
     * Clear progress indicator
     * @param {number} [windowId] - Window ID
     * @returns {Promise<Object>} Result
     */
    clear: (windowId) => ipcRenderer.invoke(IPC_CHANNELS.PROGRESS_CLEAR, { windowId }),
};
/**
...
const recentDocsAPI = {
     ...
     * @returns {Promise<Object>} Result
     */
    add: (filePath) => ipcRenderer.invoke(IPC_CHANNELS.RECENT_DOCS_ADD, { filePath }),
    /**
     * Clear all recent documents
     * @returns {Promise<Object>} Result
     */
    clear: () => ipcRenderer.invoke(IPC_CHANNELS.RECENT_DOCS_CLEAR, {}),
};
/**
...
const notificationsAPI = {
     ...
     */
    show: (options) => ipcRenderer.invoke(IPC_CHANNELS.NOTIFICATION_SHOW, options),
    /**
     * Close a notification
     * @param {string} id - Notification ID
     * @returns {Promise<Object>} Result
     */
    close: (id) => ipcRenderer.invoke(IPC_CHANNELS.NOTIFICATION_CLOSE, { id }),
    /**
     * Get notification history
     * @param {number} [limit=50] - Maximum number of entries
     * @returns {Promise<import('./common/types.ts').NotificationInfo[]>} Notification history
     */
    getHistory: (limit = 50) => ipcRenderer.invoke(IPC_CHANNELS.NOTIFICATION_GET_HISTORY, { limit }).then(r => r.history),
    /**
     * Check if notifications are allowed
     * @returns {Promise<boolean>} True if allowed
     */
    checkPermission: () => ipcRenderer.invoke(IPC_CHANNELS.NOTIFICATION_CHECK_PERMISSION, {}).then(r => r.allowed),
    /**
     * Request notification permission
     * @returns {Promise<boolean>} True if granted
     */
    requestPermission: () => ipcRenderer.invoke(IPC_CHANNELS.NOTIFICATION_REQUEST_PERMISSION, {}).then(r => r.granted),
    /**
