...
import { registerHandlers } from './ipc/bridge.ts';
import { registerLogHandlers } from './ipc/handlers/log.ts';
import { ipcSchema } from './ipc/schema.ts';
import { createWindowHandlers } from './ipc/handlers/window.ts';
import { createStoreHandlers } from './ipc/handlers/store.ts';
import { createDialogHandlers } from './ipc/handlers/dialog.ts';
import { createAppHandlers } from './ipc/handlers/app.ts';
import { secureStoreHandlers } from './ipc/handlers/secure-store.ts';
import { fileHandlers } from './ipc/handlers/files.ts';
import { dataHandlers } from './ipc/handlers/data.ts';
import { trayHandlers } from './ipc/handlers/tray.ts';
import { shortcutHandlers } from './ipc/handlers/shortcuts.ts';
import { notificationHandlers } from './ipc/handlers/notifications.ts';
import { i18nHandlers } from './ipc/handlers/i18n.ts';
import { createPluginHandlers } from './ipc/handlers/plugins.ts';
import { trayManager } from './tray.ts';
import { shortcutManager } from './shortcuts.ts';
import { setupFileHandlers, findFileInArgv } from './file-handlers.ts';
import { setupPowerMonitor } from './power.ts';
import { osHandlers } from './ipc/handlers/os.ts';
...
import { splashManager } from './splash.ts';
        ...
        try {
            ...
            if (config.osIntegration?.shortcuts?.enabled) {
                ...
                for (const [accelerator, action] of Object.entries(defaults)) {
                    const registered = shortcutManager.register(accelerator, () => {
            ...
            // Check for file arguments on startup (Windows/Linux)
            if (process.platform === 'win32' || process.platform === 'linux') {
                const filePath = findFileInArgv(process.argv);
                if (filePath) {
                    logger.info('File argument detected on startup', { filePath });
                    // File handlers will be setup, but we need to send the event once window is ready
                    // Since this is called after main window is shown in startup(), we can try now
                    const mainWindow = windowManager.getWindowByType('main');
                    if (mainWindow) {
                        fs.readFile(filePath, 'utf-8').then(content => {
                            mainWindow.webContents.send('app:file-opened', { filePath, content });
                            addRecentDocument(filePath);
                        }).catch(err => {
                            logger.error('Failed to read startup file', err);
                        });
                    }
                }
            }
                ...
                if (possibleFile) {
                    ...
                    try {
                        ...
                        if (exists) {
                            ...
                            mainWindow.webContents.send('app:file-opened', {
