...
export function validateFilePath(filePath, options = {}) {
    const { allowedExtensions = ALLOWED_EXTENSIONS, maxSize = MAX_FILE_SIZE, allowAbsolute = true, allowRelative = false, } = options;
    // Check path length
    ...
    const isForbidden = FORBIDDEN_PATHS.some(forbiddenPath => resolvedPath.toLowerCase().startsWith(forbiddenPath.toLowerCase()));
    if (isForbidden) {
        return {
            valid: false,
