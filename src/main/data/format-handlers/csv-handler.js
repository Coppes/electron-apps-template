...
import { getCsvWorkerPool } from '../../workers/worker-pool.ts';
...
export const csvHandler = {
    ...
    async import(content, options = {}) {
        const { headers = true, delimiter = ',', skipEmptyLines = true, useWorker = true } = options;
        try {
    ...
    async importStream(readStream, options = {}) {
        const { headers = true, delimiter = ',', skipEmptyLines = true } = options;
        const records = [];
