...
export class ImportExportManager {
    ...
    async export(filePath, data, options = {}) {
        try {
            ...
            if (onProgress)
                onProgress({ phase: 'start', progress: 0 });
            // Export data
            ...
            if (exported.length > STREAMING_THRESHOLD && handler.exportStream) {
                await new Promise((resolve, reject) => {
                    ...
                    handler.exportStream(data, handlerOptions, writeStream)
                        .then(() => {
                        if (onProgress)
                            onProgress({ phase: 'exporting', progress: 100, bytes: totalBytes });
                    });
                });
            }
            else {
                ...
                if (onProgress)
                    onProgress({ phase: 'exporting', progress: 100, bytes: exported.length });
            }
            if (onProgress)
    ...
    async import(filePath, options = {}) {
        try {
            ...
            if (onProgress)
                onProgress({ phase: 'start', progress: 0, total: totalSize });
            ...
            if (totalSize > STREAMING_THRESHOLD && handler.importStream) {
                ...
                if (onProgress)
                    onProgress({ phase: 'importing', progress: 100, bytes: content.length, total: totalSize });
